...
public class DeepBasaltPillarFeature extends Feature<NoneFeatureConfiguration> {
	public DeepBasaltPillarFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159446_) {
		...
		if (worldgenlevel.isWaterAt(blockpos) && !worldgenlevel.isWaterAt(blockpos.above())) {
			...
			while (worldgenlevel.isWaterAt(blockpos$mutableblockpos)) {
			...
			for (int i = -3; i < 4; ++i) {
				for (int j = -3; j < 4; ++j) {
					...
					if (randomsource.nextInt(10) < 10 - k) {
						...
						while (worldgenlevel.isWaterAt(blockpos$mutableblockpos1.setWithOffset(blockpos$mutableblockpos2, Direction.DOWN))) {
						...
						if (!worldgenlevel.isWaterAt(blockpos$mutableblockpos1.setWithOffset(blockpos$mutableblockpos2, Direction.DOWN))) {
