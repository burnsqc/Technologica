...
public class Narwhal extends WaterAnimal {
	private static final EntityDataAccessor<BlockPos> TREASURE_POS = SynchedEntityData.defineId(Narwhal.class, EntityDataSerializers.BLOCK_POS);
	private static final EntityDataAccessor<Boolean> GOT_FISH = SynchedEntityData.defineId(Narwhal.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> MOISTNESS_LEVEL = SynchedEntityData.defineId(Narwhal.class, EntityDataSerializers.INT);
	...
	public Narwhal(EntityType<? extends Narwhal> p_28316_, Level p_28317_) {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Narwhal.DolphinSwimToTreasureGoal(this));
		this.goalSelector.addGoal(2, new Narwhal.DolphinSwimWithPlayerGoal(this, 4.0D));
		this.goalSelector.addGoal(4, new RandomSwimmingGoal(this, 1.0D, 10));
		this.goalSelector.addGoal(4, new RandomLookAroundGoal(this));
		this.goalSelector.addGoal(5, new LookAtPlayerGoal(this, Player.class, 6.0F));
		this.goalSelector.addGoal(6, new MeleeAttackGoal(this, 1.2F, true));
		this.goalSelector.addGoal(8, new Narwhal.PlayWithItemsGoal());
	...
	public boolean doHurtTarget(Entity p_28319_) {
		boolean flag = p_28319_.hurt(this.damageSources().mobAttack(this), ((int) this.getAttributeValue(Attributes.ATTACK_DAMAGE)));
		if (flag) {
	...
	static class DolphinSwimToTreasureGoal extends Goal {
		private final Narwhal dolphin;
		private boolean stuck;

		DolphinSwimToTreasureGoal(Narwhal p_28402_) {
		...
		public boolean canContinueToUse() {
			...
			return !BlockPos.containing(blockpos.getX(), this.dolphin.getY(), blockpos.getZ()).closerToCenterThan(this.dolphin.position(), 4.0D) && !this.stuck && this.dolphin.getAirSupply() >= 100;
		}
		...
		public void stop() {
			BlockPos blockpos = this.dolphin.getTreasurePos();
			if (BlockPos.containing(blockpos.getX(), this.dolphin.getY(), blockpos.getZ()).closerToCenterThan(this.dolphin.position(), 4.0D) || this.stuck) {
		...
		public void tick() {
			...
			if (this.dolphin.closeToNextPos() || this.dolphin.getNavigation().isDone()) {
				...
				Vec3 vec31 = DefaultRandomPos.getPosTowards(this.dolphin, 16, 1, vec3, (float) Math.PI / 8F);
				if (vec31 == null) {
					vec31 = DefaultRandomPos.getPosTowards(this.dolphin, 8, 4, vec3, (float) Math.PI / 2F);
				}
				...
				if (vec31 != null) {
					...
					if (!level.getFluidState(blockpos).is(FluidTags.WATER) || !level.getBlockState(blockpos).isPathfindable(level, blockpos, PathComputationType.WATER)) {
						vec31 = DefaultRandomPos.getPosTowards(this.dolphin, 8, 5, vec3, (float) Math.PI / 2F);
					}
				}
	...
	static class DolphinSwimWithPlayerGoal extends Goal {
		...
		DolphinSwimWithPlayerGoal(Narwhal p_28413_, double p_28414_) {
		...
		@Override
		public boolean canUse() {
			this.player = this.dolphin.level().getNearestPlayer(Narwhal.SWIM_WITH_PLAYER_TARGETING, this.dolphin);
			if (this.player == null) {
				return false;
			} else {
	...
	class PlayWithItemsGoal extends Goal {
		...
		@Override
		public boolean canUse() {
			if (this.cooldown > Narwhal.this.tickCount) {
				return false;
			} else {
				List<ItemEntity> list = Narwhal.this.level().getEntitiesOfClass(ItemEntity.class, Narwhal.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Narwhal.ALLOWED_ITEMS);
				return !list.isEmpty() || !Narwhal.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty();
			}
		}
		...
		public void start() {
			List<ItemEntity> list = Narwhal.this.level().getEntitiesOfClass(ItemEntity.class, Narwhal.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Narwhal.ALLOWED_ITEMS);
			if (!list.isEmpty()) {
				Narwhal.this.getNavigation().moveTo(list.get(0), 1.2F);
				Narwhal.this.playSound(SoundEvents.DOLPHIN_PLAY, 1.0F, 1.0F);
			}
		...
		@Override
		public void stop() {
			ItemStack itemstack = Narwhal.this.getItemBySlot(EquipmentSlot.MAINHAND);
			if (!itemstack.isEmpty()) {
				this.drop(itemstack);
				Narwhal.this.setItemSlot(EquipmentSlot.MAINHAND, ItemStack.EMPTY);
				this.cooldown = Narwhal.this.tickCount + Narwhal.this.random.nextInt(100);
			}
		...
		private void drop(ItemStack p_28429_) {
			if (!p_28429_.isEmpty()) {
				double d0 = Narwhal.this.getEyeY() - 0.3F;
				ItemEntity itementity = new ItemEntity(Narwhal.this.level(), Narwhal.this.getX(), d0, Narwhal.this.getZ(), p_28429_);
				itementity.setPickUpDelay(40);
				itementity.setThrower(Narwhal.this.getUUID());
				float f1 = Narwhal.this.random.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * Narwhal.this.random.nextFloat();
				itementity.setDeltaMovement(0.3F * -Mth.sin(Narwhal.this.getYRot() * ((float) Math.PI / 180F)) * Mth.cos(Narwhal.this.getXRot() * ((float) Math.PI / 180F)) + Mth.cos(f1) * f2, 0.3F * Mth.sin(Narwhal.this.getXRot() * ((float) Math.PI / 180F)) * 1.5F, 0.3F * Mth.cos(Narwhal.this.getYRot() * ((float) Math.PI / 180F)) * Mth.cos(Narwhal.this.getXRot() * ((float) Math.PI / 180F)) + Mth.sin(f1) * f2);
				Narwhal.this.level().addFreshEntity(itementity);
			}
		}
	}
}