...
public class LivingEquipmentChange {
	...
	@SubscribeEvent
	public void onLivingEquipmentChangeEvent(final LivingEquipmentChangeEvent event) {
		if (event.getEntity() instanceof ServerPlayer serverPlayer) {
			...
			if (InventoryUtil.fullArmorSet(serverPlayer, TechnologicaArmorMaterial.DIVE)) {
				((IMixinMaxAir) serverPlayer).setMaxAirSupply(6000);
				TechnologicaAbilities diver = serverPlayer.getCapability(TechnologicaCapabilities.DIVER_INSTANCE).orElseThrow(NullPointerException::new);
				diver.setDiver(true);
				Technologica.CHANNEL.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new UpdateDiverCapability(true));
				return;
			}

			if (InventoryUtil.fullArmorSet(serverPlayer, TechnologicaArmorMaterial.SCUBA)) {
