...
public class PipeWrenchItem extends ToolItem implements ILink {
   ...
   public float getDestroySpeed(ItemStack stack, BlockState state) {
      ...
      return material != Material.IRON && material != Material.ANVIL && material != Material.ROCK ? super.getDestroySpeed(stack, state) : this.efficiency;
   	...
   	public ActionResultType onItemUse(ItemUseContext context) {
   		...
   		if (blockstate.matchesBlock(ModBlocks.LINE_SHAFT_HANGER.get())) {
   			setLink(blockpos, blockstate, player);
   		}
	...
	public void setLink(BlockPos posIn, BlockState stateIn, PlayerEntity playerIn) {
		playerIn.sendMessage(new StringTextComponent("test"), Util.DUMMY_UUID);
	}	
}