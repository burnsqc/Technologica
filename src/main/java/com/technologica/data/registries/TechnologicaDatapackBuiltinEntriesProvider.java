...
import com.technologica.world.level.biome.TechnologicaBiomeSources;
...
public class TechnologicaDatapackBuiltinEntriesProvider extends DatapackBuiltinEntriesProvider {
	...
	public TechnologicaDatapackBuiltinEntriesProvider(PackOutput output, CompletableFuture<Provider> registries) {
		super(output, registries, BUILDER.add(ForgeRegistries.Keys.BIOME_MODIFIERS, context -> {
			...
			TechnologicaBiomeSources.bootstrap(BuiltInRegistries.BIOME_SOURCE);
			...
			context.register(MOON_STEM, new LevelStem(moonDimensionType, new NoiseBasedChunkGenerator(new MultiNoiseBiomeSource.Preset(new ResourceLocation(Technologica.MODID, "moon"), (p_255954_) -> {
				return new Climate.ParameterList<>(ImmutableList.of(Pair.of(Climate.parameters(0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F), reference1), Pair.of(Climate.parameters(0.0F, -0.5F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F), reference1), Pair.of(Climate.parameters(0.4F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F), reference1)));
			}).biomeSource(biomes), moonNoiseGeneratorSettings)));
