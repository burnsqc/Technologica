...
public class JaguarModel<T extends JaguarEntity> extends AgeableModel<T> {
	...
	public JaguarModel() {
		...
		this.tail2.setRotationPoint(0.0F, 8.0F, -2.0F);
	...
	public void setRotationAngles(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (this.state != 3) {
			...
			if (this.state == 2) {
				...
				this.tail2.rotateAngleX = 0.7278761F + ((float) Math.PI / 10F) * MathHelper.cos(limbSwing) * limbSwingAmount;
			} else {
				this.legBackLeft.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F) * limbSwingAmount;
				...
				if (this.state == 1) {
					this.tail2.rotateAngleX = 0.7278761F + ((float) Math.PI / 4F) * MathHelper.cos(limbSwing) * limbSwingAmount;
				} else {
					this.tail2.rotateAngleX = 0.7278761F + 0.47123894F * MathHelper.cos(limbSwing) * limbSwingAmount;
				}
			}
		}
