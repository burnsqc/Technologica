package com.technologica.client.renderer.entity.vehicle;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.vertex.VertexConsumer;
import com.mojang.math.Axis;
import com.technologica.client.model.BiPlaneModel;
import com.technologica.client.model.geom.TechnologicaModelLayers;
import com.technologica.util.text.TechnologicaLocation;
import com.technologica.world.entity.vehicle.Aircraft;

import net.minecraft.client.model.ListModel;
import net.minecraft.client.renderer.MultiBufferSource;
import net.minecraft.client.renderer.RenderType;
import net.minecraft.client.renderer.entity.EntityRenderer;
import net.minecraft.client.renderer.entity.EntityRendererProvider.Context;
import net.minecraft.client.renderer.texture.OverlayTexture;
import net.minecraft.resources.ResourceLocation;

public final class SubmersibleRenderer extends EntityRenderer<Aircraft> {
	private static final ResourceLocation TEXTURE = new TechnologicaLocation("textures/entity/airplane/biplane.png");
	private final ListModel<Aircraft> listmodel;

	public SubmersibleRenderer(Context context) {
		super(context);
		listmodel = new BiPlaneModel(context.bakeLayer(TechnologicaModelLayers.SUBMERSIBLE));
	}

	@Override
	public ResourceLocation getTextureLocation(Aircraft entity) {
		return TEXTURE;
	}

	@Override
	public void render(Aircraft biplane, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		poseStack.pushPose();
		poseStack.translate(0.0F, 0.375F, 0.0F);
		poseStack.mulPose(Axis.YP.rotationDegrees(180.0F - entityYaw));
		// poseStack.mulPose(Axis.XP.rotationDegrees(-biplane.getXRot()));
		// poseStack.mulPose(Axis.ZP.rotationDegrees(-biplane.roll));
		poseStack.scale(-1.0F, -1.0F, 1.0F);
		poseStack.mulPose(Axis.YP.rotationDegrees(90.0F));
		listmodel.setupAnim(biplane, entityYaw, partialTicks, -0.1F, 0.0F, 0.0F);
		VertexConsumer vertexconsumer = buffer.getBuffer(RenderType.entityTranslucent(getTextureLocation(biplane)));
		listmodel.renderToBuffer(poseStack, vertexconsumer, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
		poseStack.popPose();
		super.render(biplane, entityYaw, partialTicks, poseStack, buffer, packedLight);
	}
}
