...
public class RaccoonModel<T extends Raccoon> extends QuadrupedModel<T> {
	...
	if (entityIn.isBegging()) {
	    ...
	    this.leftFrontLeg.xRot = Mth.sin(0.75F * ((float) entityIn.tickCount + partialTick)) - (float) Math.PI / 3;
	    this.rightFrontLeg.xRot = Mth.sin(0.75F * ((float) entityIn.tickCount + partialTick)) - (float) Math.PI / 3;
